...
class Paynet
{
	...
	public $expiryDate;
	public $externalDate;
	public $linkUrlSucces;
	public $linkUrlCancel;

	public $services = array();
	public $customer = array();

	/**
	 * @param string $merchant_code Код продавца Paynet
	 * @param string $merchant_secret_key Секретный ключ продавца Paynet
	 * @param string $merchant_sale_area_code
	 * @param string $merchant_user Пользователь продавец Paynet
	 * @param string $merchant_password Пароль пользователя продавца Paynet
	 */
	public function __construct($merchant_code, $merchant_secret_key, $merchant_sale_area_code, $merchant_user, $merchant_password)
	{
	 ...
	 *
	 * @param integer $payment_mode 0 - тест | 1 - реальный режим
	 * @return void
	 */
	 ...
	 * @return void
	 */
	public function setServices($name, $description, $products)
	{
	 ...
	 * @return void
	 */
	public function setCustomer($code, $nameFirst, $nameLast, $phoneNumber, $email, $country, $city, $address)
	{
		$customer = [
			"Code" => $code,
			"NameFirst" => $nameFirst,
	 ...
	 * @return void
	 */
	public function setLinkUrlSucces($url)
	{
		$this->linkUrlSucces = $url;
	}
	 ...
	 * @return void
	 */
	public function setLinkUrlCancel($url)
	{
		$this->linkUrlCancel = $url;
	}
	 ...
	 * @return datetime
	 */
	private function expiryDate($addHours = 4)
	{
	 ...
	 * @return object
	 */
	private function callApi($path, $method, $params = array(), $headers = array())
	{
	...
	public function getToken()
	{
		...
		if ($api->code == PaynetResult::CODE_SUCCESS) {
			if (array_key_exists("access_token", $api->data)) {
				...
				$result->data = [
					"Authorization: Bearer " . $api->data["access_token"],
	 ...
	 * @return object
	 */
	public function initServerServerPayment()
	{
			...
			if ($api->code == PaynetResult::CODE_SUCCESS) {
				if (array_key_exists("Code", $api->data)) {
					...
					$form .= '<input type="hidden" name="operation" value="' . $api->data['PaymentId'] . '"/>' .
						'<input type="hidden" name="LinkUrlSucces" value="' . $this->linkUrlSucces . '"/>' .
						'<input type="hidden" name="LinkUrlCancel" value="' . $this->linkUrlCancel . '"/>' .
						...
						'<button>Продолжить оплату</button>' .
						'<div>перенаправление через 3 сек.</div>' .
						'</form>' .
